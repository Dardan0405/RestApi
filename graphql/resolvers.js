...
const { updatePost, deletePost } = require("../controllers/feed")

module.exports={
    createUser: async function({userInput}, req){
        // const email = args.userInput.emai;
        const errors = [];
...
}, 'secretsupersupersuper',{ expiresIn: '1h'})
...
if(errors.length >0){
    ...
    if(postInput.imageUrl !== 'udnefined'){
    ...
    deletePost: async function({id},re1){
        if(!req.isAuth){
            const error = new Error('Not Auth');
            error.code = 403
            throw error
